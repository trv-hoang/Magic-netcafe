package com.netcafe.ui;

import java.awt.Color;
import java.awt.Font;

public class ThemeConfig {
    // Colors
    public static final Color PRIMARY = new Color(99, 102, 241); // Indigo
    public static final Color SECONDARY = new Color(16, 185, 129); // Emerald
    public static final Color ACCENT = new Color(245, 158, 11); // Amber
    public static final Color DANGER = new Color(239, 68, 68); // Red
    public static final Color SUCCESS = new Color(34, 197, 94); // Green

    public static final Color BG_MAIN = new Color(248, 250, 252); // Slate 50
    public static final Color BG_PANEL = Color.WHITE;
    public static final Color TEXT_PRIMARY = new Color(30, 41, 59); // Slate 800
    public static final Color TEXT_SECONDARY = new Color(100, 116, 139); // Slate 500

    // Fonts
    public static final Font FONT_HEADER = new Font("SansSerif", Font.BOLD, 24);
    public static final Font FONT_SUBHEADER = new Font("SansSerif", Font.BOLD, 18);
    public static final Font FONT_BODY = new Font("SansSerif", Font.PLAIN, 14);
    public static final Font FONT_BODY_BOLD = new Font("SansSerif", Font.BOLD, 14);
    public static final Font FONT_SMALL = new Font("SansSerif", Font.PLAIN, 12);
    public static final Font FONT_MONO = new Font("Monospaced", Font.PLAIN, 14);

    // Dimensions
    public static final int CORNER_RADIUS = 16;
    public static final int PADDING_SMALL = 8;
    public static final int PADDING_MEDIUM = 16;
    public static final int PADDING_LARGE = 24;
}
